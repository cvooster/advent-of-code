...
import advent_of_code.year_2018.day_01.solution as s01

FILENAME = "input.txt"
FILENAMES_EXAMPLES = tuple(f"input_example_{i+1}.txt" for i in range(8))


class TestSolution(unittest.TestCase):
    """Tests for the solution of puzzle 2018-01."""
    ...
    def test_aoc_verified_part_1(self):
        ...
        actual = s01.solve_p1(FILENAME)
        self.assertEqual(actual, 578)

    def test_example_part_2(self):
        """Test whether part 2 outcomes for the examples are as expected."""
        filenames_selection = tuple(FILENAMES_EXAMPLES[i] for i in (0, 4, 5, 6, 7))
        expecteds = (2, 0, 10, 5, 14)
        for filename, expected in zip(filenames_selection, expecteds):
            with self.subTest(filename=filename):
